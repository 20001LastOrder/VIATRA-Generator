...
public class GraphPermutationTest {
    ...
    public void testUniqueGraph() {
        ...
        IncrementalEncoder encoder = new IncrementalEncoder(new Encoder.NativeHashCertificatePropagator());
        IStateCode coder = new ArrayStateCode();
        for (Graph graph : graphs) {
            Object code = encoder.calculateStateCode(graph, coder);
    ...
    public void testUniqueGraph_4_Directed_SelfLoop() {
        ...
        Encoder encoder = new Encoder(new Encoder.NativeHashCertificatePropagator());
        IStateCode coder = new AdjacencyStateCode(10);
        for (Graph graph : graphs) {
            Integer code = encoder.calculateStateCode(graph, coder).hashCode();
            if (!map.containsKey(code)) {
                map.put(code, new ArrayList<>());
            }
            List<Graph> isomorphicGraphs = map.get(code);
